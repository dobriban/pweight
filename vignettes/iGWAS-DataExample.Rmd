...
w <- res_default$w
P_weighted <- c4d$P/w
cat(c("Number of genome-wide significant SNPs (P < 5e-8) with weighting: ", sum(P_weighted < 5e-8) ))
```
...
Other packages can be used for downstream analysis of significant SNPs. We can use qqman to derive a global visualization of significant P values, before and after weighting:
...
```{r}
#EDITS NEEDED: Would like to use genome-side sig rather than bonferroni, and phi=0
phi=seq(0.5,4,by=0.5)
